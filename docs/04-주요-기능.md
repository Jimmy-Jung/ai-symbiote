...
flowchart TD
    ...
    F["PostToolUse(Read|Write|Edit)"] --> GT["gateguard-tracker.sh"]
    F2["PostToolUse(Read|Skill)"] --> UT["usage-tracker.sh"]
    F3["PostToolUse(Write|Edit)"] --> G["harness-learn / security-guard / suggest-compact"]
...
| PostToolUse(Read\|Write\|Edit) | gateguard-tracker.sh | 읽은 파일과 성공한 쓰기 대상 추적 |
| PostToolUse(Read\|Skill) | usage-tracker.sh | 스킬/명령 사용량 기록 |
| PostToolUse(Write\|Edit) | harness-learn / security-guard / suggest-compact | 패턴 학습 + 보안 검사 + compact 제안 |
