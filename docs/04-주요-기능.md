...
flowchart TD
    ...
    DISP --> SC["suggest-compact.sh"]
    D3["PreToolUse(*)"] --> MH["mcp-health-check.sh"]

    F["PostToolUse(Read)"] --> GT["gateguard-tracker.sh"]
    F2["PostToolUse(Write|Edit)"] --> G["harness-learn / security-guard"]
    F3["PostToolUseFailure(*)"] --> MF["mcp-health-failure.sh"]
...
| PreToolUse(Edit\|Write) | pre-edit-write-dispatcher.sh | Config Protection → GateGuard → Compact 제안 |
| PreToolUse(*) | mcp-health-check.sh | 장애 MCP 서버 차단 |
| PostToolUse(Read) | gateguard-tracker.sh | 읽은 파일 추적 |
| PostToolUse(Write\|Edit) | harness-learn / security-guard | 패턴 학습 + 보안 검사 |
| PostToolUseFailure(*) | mcp-health-failure.sh | MCP 실패 기록 |
| Stop | cost-tracker.sh + instinct-observer.sh | 세션 메트릭 + 관찰 기록 |
