...
flowchart TD
    ...
    style C2 fill:#e5e7eb,stroke:#6b7280,color:#111827
...
flowchart TD
    ...
    H --> I["질문 N=5 생성<br/>(--pool-size로 조정)"]
    ...
    M -->|GAMING_DETECTED| O["Bounded rewrite<br/>(기본 max_rewrites=2)"]
    O --> K
    ...
    style I fill:#fef3c7,stroke:#b45309,color:#111827
    style J fill:#dbeafe,stroke:#1d4ed8,color:#111827
    style K fill:#e5e7eb,stroke:#6b7280,color:#111827
    style L fill:#dbeafe,stroke:#1d4ed8,color:#111827
    style M fill:#dbeafe,stroke:#1d4ed8,color:#111827
    style N fill:#dcfce7,stroke:#15803d,color:#111827
    style O fill:#fee2e2,stroke:#b91c1c,color:#111827
```
...
| Reviewer + Judge | `/verify` 스킬 (사용자 명시적 호출) | ~60s × N × (1+max_rewrites) | `shared/skills/verify/SKILL.md` |
...
| V2b | LLM Judge verifiability | 저자와 다른 벤더의 judge. 기본은 evidence packet만 사용, repo scan off | **필수 방어선** |
| V2c | 질문 랜덤화 | reviewer N=5 기본 → 저자 K=3 무작위 | 필수 |

### 비용 상한 계약

`/verify`는 adversarial 검증이라 실패 시 재질문/재답변이 필요하지만, 무제한
반복은 금지합니다.

| 옵션 | 기본값 | 의미 |
|------|--------|------|
| `--max-batch` | `10` | 한 번에 처리할 queue entry 수 |
| `--max-rewrites` | `2` | 첫 시도 이후 허용할 answer/rewrite 라운드 수 |
| `--pool-size` | `5` | reviewer 질문 풀 크기 |
| `--judge-strictness` | `standard` | factual claim은 엄격 검증, 명시적 engineering judgment는 검증 대상에서 제외 |
| `--judge-repo-scan` | off | judge가 evidence packet 밖의 repo 파일을 탐색할지 여부 |

예상 비용은 happy path가 아니라 `entry 수 × (1 + max_rewrites)` 기준으로
표시합니다. budget이 소진되면 FAIL artifact에 `budget_exhausted: true`를
기록하고 queue entry는 유지합니다.
