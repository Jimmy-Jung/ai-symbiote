...
> Telegram, Slack, Discord를 통해 Claude/Codex 백엔드를 원격으로 다루는 메신저 브릿지
...
Telegram/Slack/Discord
        ↕ (Bot API)
   Node.js 봇 서버 (맥에서 실행)
        ↕ (filesystem watcher + spawn)
   Claude CLI / Codex CLI
```

봇 서버가 맥에서 상주하며, 메신저 메시지를 받아 `claude -p` 또는 `codex exec`를 실행하고 결과를 회신합니다. 상태 파일은 `~/ai-symbiote/{slug}/messenger/` 아래에서 공유되고, watcher가 `notifications/`, `approvals/`, `commands/` 디렉터리를 계속 감시합니다.
...
대화형 마법사가 플랫폼 선택, 토큰 입력, `shared/messenger-bridge/` 빌드, `bot.pid` 생성까지 한 번에 안내합니다.
...
`security.permissionLevel`로 브리지에서 호출하는 AI CLI의 권한을 제한합니다. Claude에는 도구 허용/차단 인자로, Codex에는 sandbox 모드로 매핑됩니다.
...
| 명령 | 설명 |
|------|------|
| 자유 텍스트 | 현재 백엔드로 전달. Claude는 세션 유지, Codex는 요청 단위 실행 |
| `/claude <질문>` | Claude에 질문 (백엔드를 claude로 전환) |
| `/codex <질문>` | Codex에 질문 (백엔드를 codex로 전환) |
| `/chat <질문>` | 현재 백엔드 유지한 채 질문 |
...
Codex 백엔드는 현재 브리지에서 요청마다 새 `codex exec` 프로세스로 실행됩니다. 긴 대화 맥락을 이어가는 용도보다는 단발성 코드 리뷰, 분석, 초안 생성에 더 잘 맞습니다.

### 백엔드 전환

`/claude` 또는 `/codex`로 백엔드를 전환하면, 이후 자유 텍스트도 해당 백엔드로 유지됩니다. `/new`나 `/disconnect`를 호출하면 백엔드 상태와 Claude 세션 정보가 함께 초기화됩니다.
