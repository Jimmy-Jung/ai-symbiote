...
name: deep-search
user-invocable: true
description: 다중 전략 코드베이스 심층 탐색 워크플로우. Grep, Glob, subagent(explorer)를 병렬로 실행하여 심볼, 패턴, 의존성을 종합적으로 분석. This skill should be used when performing deep codebase analysis or searching for complex code patterns and dependencies.
---
...
Track B -- 파일 패턴 (Glob): 네이밍 컨벤션, 디렉터리 구조 기반 파일 탐색
Track C -- 심층 탐색 (Agent Explore): 넓은 범위의 코드베이스 탐색
...
### Step 4: 구조화 리포트
발견 파일, 의존성 관계, 주요 발견, 영향 범위, 추가 탐색 권장 항목을 정리합니다.
...
이 섹션은 synapse 오케스트레이터가 Scout 서브에이전트를 스폰할 때 프롬프트에 주입됩니다.

Scout는 위의 4-Step 워크플로우와 3-Track 병렬 검색 전략을 따릅니다.
시나리오별 전략을 상황에 맞게 선택하여 적용하세요.
