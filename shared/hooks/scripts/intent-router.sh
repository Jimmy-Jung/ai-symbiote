...
COMPACT_HINTS=""
if command -v jq >/dev/null 2>&1; then
  COMPACT_HINTS=$(jq -r '.categories | to_entries[] | select(.key != "none") | "\(.key): \(.value.signals[0]) → /\(.value.recommended_skills | join(", /"))"' "$HINTS_FILE" 2>/dev/null) || COMPACT_HINTS=""
elif command -v python3 >/dev/null 2>&1; then
  COMPACT_HINTS=$(python3 -c "
import json
